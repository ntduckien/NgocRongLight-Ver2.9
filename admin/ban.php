            ...
            <main>
                <div class="row">
                    <div class="col-md-9 pb-3 pt-2">
                        <h3>Khoá Tài Khoản - Máy Chủ 1</h3>
                        <p class="text-muted">Khoá tài khoản người chơi có vi phạm điều luật của máy chủ, tài khoản sẽ
                            được sắp xếp vào bảng phong thần </p>
                        <hr>
                        <table class="table">
                    </div>
                </div>
                    ...
                    <div class="table-responsive">
                        <div class="table-responsive">
                            ...
                            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                                ...
                                // Truy vấn cơ sở dữ liệu để kiểm tra tài khoản và trạng thái ban
                                $sql = "SELECT * FROM account WHERE username = '$_username'";
                                $result = $conn->query($sql);

                                if ($result->num_rows == 0) {
                                    // Thông báo lỗi nếu tài khoản không tồn tại
                                    $_alert = '<div class="alert alert-danger">Tên tài khoản không tồn tại!</div>';
                                } else {
                                    ...
                                    if ($is_banned === '1') {
                                        ...
                                        $_alert = '<div class="alert alert-warning">Tài khoản này đã bị khóa!</div>';
                                    } else {
                                        // Nếu tài khoản chưa bị khóa, tiến hành khoá tài khoản và set giá trị của cột ban thành 1 trong cơ sở dữ liệu.
                                        $sql2 = "UPDATE account SET ban = '1' WHERE username = '$_username'";
                                        if ($conn->query($sql2) === TRUE) {
                                            // Thông báo thành công khi khoá tài khoản thành công
                                            $_alert = '<div class="alert alert-success">Khoá tài khoản thành công!</div>';
                                        } else {
                            ...
                            <form method="POST">
                                ...
                                <button class="btn btn-outline-primary" type="submit">Khoá</button>
                            </form>
                        </div>
